...
data_can_be_validated = function(var) {
  if (is.null(var)) return(F)
  if (is.na(var)) return(F)
  if (str_trim(tolower(as.character(var))) %in% c("", "-1", "empty")) return(F)
  var = str_trim(var)
  if (is_date(var)) return(date_can_be_validated(var))
  T
}
...
data_missing = function(var) {
  if (is.null(var)) return(T)
  if (is.na(var)) return(T)
  if (stringr::str_trim(var) == "") return(T)
  F
}
